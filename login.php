...
if (isset($_SESSION["username"])) {
    if ($_SESSION["role"] == "admin") {
        header("Location: http://localhost/phpmyadmin/public_html/admin.php");
    } else {
        header("Location: http://localhost/phpmyadmin/public_html/uye.php");
    }
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($name) && !empty($pass)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $_SESSION["role"] = "admin"; // Admin olarak belirle
            header("Location: http://localhost/phpmyadmin/public_html/admin.php");
            exit;
        }

        // Eğer admin bulunmazsa, üye tablosuna bak
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $_SESSION["username"] = $name;
            $_SESSION["role"] = "uye"; // Üye olarak belirle
            header("Location: http://localhost/phpmyadmin/public_html/uye.php");
            exit;
        }
...
<script>// Sayfa yüklendiğinde animasyonu başlat
  ...
  <body data-home-page="Sayfa-2.html" data-home-page-title="ŞAFAK YAPI" data-path-to-root="/" data-include-products="false" class="u-body u-xl-mode" data-lang="tr"><header class="u-clearfix u-header u-header" id="sec-0bfd"><div class="u-clearfix u-sheet u-valign-middle u-sheet-1">
    ...
    <div class="panel">
        ...
        <p>Üye Değil misin ?<br><a href="http://localhost/phpmyadmin/public_html/singin.php">Kayıt Ol!</a></p>
    </div>
